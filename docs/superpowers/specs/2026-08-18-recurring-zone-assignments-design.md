...
RLS: enable on the table. Supervisors can select/insert/update/delete rows where `company_id = get_my_company_id()` and `get_my_role() = 'supervisor'` — this mirrors the actual existing `job_zones` policies exactly (verified directly against the live policies via `pg_policies`), not the stricter `supervisor_facilities`-scoped pattern used only by the `facilities` table itself. `supervisor_facilities` gates which *facilities* a supervisor can see at all (so in practice they won't navigate to a facility's Recurring Schedule screen unless assigned to it), but `job_zones`/`jobs` access — and now `recurring_zone_rules` access, for consistency — is company-wide once you have a row's id. Matching the sibling table's existing trust boundary is safer than inventing a new, inconsistent one; tightening this is a separate, pre-existing consideration out of scope here (Do Not Touch: RLS policy changes need a full security review).
