...
RLS: enable on the table, all four policies `to authenticated`. SELECT/DELETE: `company_id = get_my_company_id() and get_my_role() = 'supervisor'`, company-wide once you have a row's id — matching the `job_zones` read/delete trust boundary. INSERT/UPDATE additionally require `exists (select 1 from facilities f where f.id = facility_id and f.company_id = get_my_company_id())` and the same check against `profiles` for `cleaner_id`.

**Correction (post-implementation, via final branch review):** the first version of this section claimed the write policies "mirror `job_zones` exactly" using only the bare `company_id` check. That was wrong: `job_zones` has no `company_id` of its own and is constrained *transitively* through `jobs.company_id`, which itself traces back to a company-scoped `facility_id` at job-creation time — so a `job_zones` row can never point at a foreign company's facility. `recurring_zone_rules` carries its own self-declared `company_id`, and the bare check let a supervisor set that correctly while `facility_id`/`cleaner_id` pointed at another company's rows entirely — a write-integrity gap that would let an authenticated supervisor create rules against a foreign facility/cleaner (exploitable only with a foreign UUID obtained out of band; not a read leak, and the app had exactly 1 company/3 supervisors at the time, so no practical exposure occurred). Fixed while the table still had 0 rows, via a follow-up migration (`tighten_recurring_zone_rules_rls`) adding the `EXISTS` checks above and reissuing all four policies `to authenticated` (they had defaulted to `to public`, which was not itself exploitable — `get_my_role()` returns `NULL` for anonymous requests, so the `= 'supervisor'` check already excluded `anon` — but was inconsistent with every sibling supervisor policy on `jobs`/`job_zones`/`facilities`).
...
5. **Per facility group**:
   ...
   b. **If none found** — insert a new `jobs` row: `facility_id`, `company_id`, `scheduled_date = today`, `status = 'scheduled'`, `supervisor_id = <rule.created_by>` (use the first rule in the group's `created_by` — if a facility somehow has rules created by different supervisors, any one of them is an acceptable owner).

   **Correction (post-implementation, via final branch review) — `supervisor_id` here IS access-control-relevant, not cosmetic as originally claimed:** the live `jobs` SELECT policy is `supervisor_id = auth.uid()`, not company- or `supervisor_facilities`-scoped, and `FacilityZonesView.load()` applies no additional filter — it relies entirely on that policy. So a job WF-16 creates with `supervisor_id = <rule author>` is invisible to every *other* supervisor who opens that facility, and the "no duplicate-job path" conclusion in the Non-goals section above only holds when the same supervisor is on both sides. A different supervisor opening the facility that day would see "Start Today's Shift" and, if clicked, create a second job for the same facility/date with the cleaner's zones split across both. This is pre-existing behavior for manually-created jobs too, but a machine-authored job attributed to whoever happened to write the rule makes the collision meaningfully more likely. **Resolve this before building WF-16**, via one of: (a) broaden the `jobs` SELECT policy to company scope (needs its own full security review per Do Not Touch), or (b) have WF-16 — which runs as `service_role` and bypasses RLS — reconcile against *any* existing same-day job for the facility regardless of `supervisor_id`, not just ones it can see through a normal session.
...
No new top-level route. `Jobs.tsx`'s existing facility-zone-management screens (`StartShiftScreen`, `ShiftBuilderScreen`, `AddZoneScreen`, `ZoneEditScreen`) are all dispatched from the single `/supervisor/jobs` route via an `?action=` query param read in the `Jobs()` component (`src/pages/supervisor/Jobs.tsx:1650-1669`), not separate routes. This screen follows the same convention: `/supervisor/jobs?facility=<id>&action=recurring`. List vs. add vs. edit within that screen use local component state, not further query params — this is a supervisor config screen, not something that needs deep-linkable sub-states.
...
Add a "Recurring Schedule" link/button on the existing facility Jobs view (`FacilityZonesView` in `Jobs.tsx`) navigating to `/supervisor/jobs?facility=<id>&action=recurring`. Exact placement: near the existing zone-management actions, visible regardless of whether today's job exists yet (recurring schedule is facility-level config, independent of any single day's job).
