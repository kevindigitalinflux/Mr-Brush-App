...
async function downloadImage(src: string, fallbackName: string) {
  try {
    ...
    a.href = blobUrl
    a.download = src.split('/').pop()?.split('?')[0] || fallbackName
...
export function ImageViewer({ src, alt = 'Image', type = 'image', onClose }: Props) {
  ...
  return createPortal(
    ...
    >
      {/* Download button */}
      <button
        onClick={(e) => { e.stopPropagation(); void downloadImage(src, type === 'video' ? 'evidence-video.webm' : 'evidence-photo.jpg') }}
        aria-label={type === 'video' ? 'Download video' : 'Download image'}
